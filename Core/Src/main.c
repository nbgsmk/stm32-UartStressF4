...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	if (huart->Instance == USART1) {
		...
		xQueueSendFromISR(quToSenzorHandle, &rxByte, &xHigherPriorityTaskWoken);
	...
	if (huart->Instance == USART2) {
		...
		BaseType_t xHigherPriorityTaskWoken = pdFALSE;
		receivedChar = (uint8_t) (huart->Instance->DR & 0xFF);
		xQueueSendFromISR(quToRpiHandle, &receivedChar,
