...
const osThreadAttr_t Blinker_attributes = { .name = "Blinker", .stack_size = 128 * 4, .priority = (osPriority_t) osPriorityLow, };
/* Definitions for usbTerminal */
osThreadId_t usbTerminalHandle;
const osThreadAttr_t usbTerminal_attributes = { .name = "usbTerminal", .stack_size = 1000 * 4, .priority = (osPriority_t) osPriorityLow, };
/* Definitions for taskToRpi */
osThreadId_t taskToRpiHandle;
const osThreadAttr_t taskToRpi_attributes = { .name = "taskToRpi", .stack_size = 256 * 4, .priority = (osPriority_t) osPriorityNormal, };
/* Definitions for taskToSensor */
osThreadId_t taskToSensorHandle;
const osThreadAttr_t taskToSensor_attributes = { .name = "taskToSensor", .stack_size = 256 * 4, .priority = (osPriority_t) osPriorityNormal, };
/* Definitions for blinkRpi */
osThreadId_t blinkRpiHandle;
const osThreadAttr_t blinkRpi_attributes = { .name = "blinkRpi", .stack_size = 128 * 4, .priority = (osPriority_t) osPriorityLow, };
/* Definitions for blinkSenzor */
osThreadId_t blinkSenzorHandle;
const osThreadAttr_t blinkSenzor_attributes = { .name = "blinkSenzor", .stack_size = 128 * 4, .priority = (osPriority_t) osPriorityLow, };
...
const osMutexAttr_t myRecursiveMutex01_attributes = { .name = "myRecursiveMutex01", .attr_bits = osMutexRecursive, };
...
int main(void) {
	...
	/* creation of timer02 */
	timer02Handle = osTimerNew(timer02Callback, osTimerOnce, NULL, &timer02_attributes);
	...
	quToSenzorHandle = osMessageQueueNew(128, sizeof(uint8_t), &quToSenzor_attributes);

	/* creation of quToRpi */
	quToRpiHandle = osMessageQueueNew(128, sizeof(uint8_t), &quToRpi_attributes);
	...
	usbTerminalHandle = osThreadNew(usbTerminalStart, NULL, &usbTerminal_attributes);
...
static void MX_GPIO_Init(void) {
	...
	/*Configure GPIO pins : SW_delay_Pin SW_dropByte_Pin SW_insertByte_Pin SW_dataError_Pin
	 SW_headerError_Pin */
	GPIO_InitStruct.Pin = SW_delay_Pin | SW_dropByte_Pin | SW_insertByte_Pin | SW_dataError_Pin | SW_headerError_Pin;
...
void usbTerminalStart(void *argument) {
	...
	/* Infinite loop */
	for (;;) {
//		HAL_UART_Transmit(&UART_RPI, (uint8_t*) "1", 1, 10);
//		HAL_UART_Transmit(&UART_SENZOR, (uint8_t*) "2", 1, 10);

//		uint8_t msg1[] = "abc";
//		CDC_Transmit_FS(msg1, sizeof(&msg1));
//		CDC_Transmit_FS("xyz", sizeof("xyz"));
